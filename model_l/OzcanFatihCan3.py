# -*- coding: utf-8 -*-
"""
Created on Sun Jun 11 03:04:02 2023
...
weights_l = 'l'  # yolov5l modeli kullanılacak

data_dir = Path('C:/Users/PC/NesneTanıma/model_l/Dataset')
img_dir = data_dir / 'images'
train_txt = data_dir / 'train.txt'
test_txt = data_dir / 'test.txt'

# Sonuçların kaydedileceği dizin
results_dir_l = data_dir / 'results_l'
results_dir_l.mkdir(exist_ok=True)

# GPU kullanımı kontrolü
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# YOLOv5 modelini yükle
model_l = torch.hub.load(str(yolov5_dir), 'custom', path=str(yolov5_dir / 'weights' / f'yolov5{weights_l}.pt'), source='local').to(device)
model_l.conf = 0.25  # Algılama eşiği (confidence threshold) ayarı
...
def predict_l():
    ...
    with open(results_dir_l / 'modelLtahmini.txt', 'a') as f:
        for line in tqdm(test_lines, desc='Predicting'):
            img_path = line.strip()
            img = Image.open(img_path).convert('RGB')
            results = model_l(img)
            results.print()  # Tahmin sonuçlarını konsola yazdır
            save_path = str(results_dir_l / Path(img_path).name)
